...
from tricks.scconv import ScConv


from mmengine.model import weight_init
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, act_num=3, stride=2, deploy=False, ada_pool=None):
        ...
        self.deploy = deploy
        if self.deploy:
            self.conv = ScConv(dim, dim_out, kernel_size=1)
        else:
            self.conv1 = nn.Sequential(
                ScConv(dim, dim, kernel_size=1),
                nn.BatchNorm2d(dim, eps=1e-6),
            )
            self.conv2 = nn.Sequential(
                ScConv(dim, dim_out, kernel_size=1),
...
@BACKBONES.register_module(name="VanillaNet",force=True)
class VanillaNet(BaseBackbone):
    def __init__(self, in_chans=3, num_classes=3, dims=[96, 192, 384, 768], 
        ...
        if self.deploy:
            self.stem = nn.Sequential(
                ScConv(in_chans, dims[0], kernel_size=4, stride=stride, padding=padding),
            ...
            self.stem2 = nn.Sequential(
                ScConv(dims[0], dims[0], kernel_size=1, stride=1),
                nn.BatchNorm2d(dims[0], eps=1e-6),
        ...
        if self.deploy:
            ...
            self.cls2 = nn.Sequential(
                ScConv(num_classes, num_classes, 1)
            )
        
        self.apply(self._init_weights)

    def _init_weights(self, m):
        if isinstance(m, (ScConv, nn.Linear)):
...
@BACKBONES.register_module(name="VanillaNet_10",force=True)
...
@BACKBONES.register_module(name="VanillaNet_11",force=True)
...
@BACKBONES.register_module(name="VanillaNet_13",force=True)
def vanillanet_13(pretrained=True, in_22k=False, **kwargs):
    model = VanillaNet(

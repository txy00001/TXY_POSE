...
def BasicConv(filter_in, filter_out, kernel_size, stride=1, pad=None):
    ...
    return nn.Sequential(OrderedDict([
        ("conv", ScConv(filter_in, filter_out, kernel_size=kernel_size, stride=stride, padding=pad, bias=False)),
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, filter_in, filter_out):
        ...
        self.conv1 = ScConv(filter_in, filter_out, 3, padding=1)
        self.bn1 = nn.BatchNorm2d(filter_out, momentum=0.1)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = ScConv(filter_out, filter_out, 3, padding=1)
...
class ASFF_2(nn.Module):
    def __init__(self, inter_dim=512):
        ...
        self.weight_levels = ScConv(compress_c * 2, 2, kernel_size=1, stride=1, padding=0)
...
class ASFF_3(nn.Module):
    def __init__(self, inter_dim=512):
        ...
        self.weight_levels = ScConv(compress_c * 3, 3, kernel_size=1, stride=1, padding=0)
...
class ASFF_4(nn.Module):
    def __init__(self, inter_dim=512):
        ...
        self.weight_levels = ScConv(compress_c * 4, 4, kernel_size=1, stride=1, padding=0)
...
@MODELS.register_module(name="TXYAFPN",force=True)
class TXYAFPN(nn.Module):
    def __init__(self,
                 in_channels=[256, 512, 1024, 2048],###通道数和rtmpose的backbone保持一致
                 out_channels=256):
        super(TXYAFPN, self).__init__()

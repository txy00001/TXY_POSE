...
from lights.tools.object_detection.yolox import YOLOX
from lights.tools.pose_estimation.rtmpose import RTMPose
from lights.tools.solution.types import BodyResult, Keypoint, PoseResult
...
class Castbody:
    ...
    def __init__(
            self,
            det: str = '/home/txy/code/CastPose/onnx/det_onnx/yolox_det.onnx',
    ...
    def format_result(keypoints_info: np.ndarray) -> List[PoseResult]:
        ...
        #     face = format_keypoint_part(instance[24:92])###wholebody


        for instance in keypoints_info:
            body_keypoints = format_keypoint_part(
                instance[:6]) or ([None] * 6)##只有双手六个关键点
            left_hand = format_keypoint_part(instance[92:113])
            right_hand = format_keypoint_part(instance[113:134])
            face = format_keypoint_part(instance[24:92])##castpose

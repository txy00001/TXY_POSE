...
@TRANSFORMS.register_module(name="DR",force=True)
class DR(BaseTransform):
    """
数据预处理部分
    """
    ...
    def generate_sa_simdr(self, cfg, joints, joints_vis):
        ...
        for joint_id in range(num_joints):
            ...
            target_weight[joint_id] = self.adjust_target_weight(cfg, joints[joint_id], target_weight[joint_id],
                                                                tmp_size)
            if target_weight[joint_id] == 0:
                continue
    ...
    def __call__(self, results):
        ...
        if self.encoding == 'MSRA':
            ...
            target_weight, filtered_joints = self.filter_target_simdr(cfg, joints_split.copy(), joints_3d_visible,
                                                                      cfg['image_size'] * self.simdr_split_ratio)
